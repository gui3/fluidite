...
var semantics = {
  ...
  Glossary: function ( options=undefined ) {
    ...
    this.morphemeRegex = function() {
      var regex = new semantics.Regex();
      for (var i=0; i< this.elements.length; i++) {
        var element = this.elements[i];
        if ( element.morpheme == true ) {
          regex.addMorpheme(element);
        }
      }
      return regex;
    };
  ...
  Regex: function () {
    ...
    this.parse =  function (script, max = -1) {
        ...
        morphemeDOM.length !== max )
      {
        ...
        morphemeElement.sense = "";
        morphemeElement.groups = [];

        for ( var i = 1; i<array.length; i++ ) {
          if ( array[i] == array[0] ) {
            morphemeElement.groups.push(i);
            morphemeElement.sense += "."+this.groupNames[i];
            //break;
...
.add({
  type:"Charset",
  tags:"operator wordMarker label get",
  syntax:"#",
  morpheme: true
})
.add({
  type:"Charset",
  tags:"operator wordMarker label go",
  syntax:"@",
  morpheme: true
})
.add({
